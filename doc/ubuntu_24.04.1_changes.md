...
- Ensure fixed-width integer types are available during builds with GCC 13.
  - Files: erizo/src/erizo/CMakeLists.txt, erizo/src/third_party/webrtc/src/CMakeLists.txt
  - Change: add `-include cstdint` to C++ compile flags
- Build workaround without source edits (forces cstdint into all translation units):
  - Command: CXXFLAGS="-include cstdint" scripts/installErizo.sh
